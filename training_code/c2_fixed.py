...
def load_tf_checkpoint(checkpoint_path, network):
    """Properly load trained weights from TensorFlow checkpoint INCLUDING Dale's law signs"""
...
def convert_to_pynn_format(checkpoint_path, data_dir, output_h5, n_neurons=51978, n_input=17400):
    ...
    # ============ CRITICAL FIX: Apply Dale's law signs ============
    print("\n" + "=" * 80)
    print("APPLYING DALE'S LAW SIGNS")
    print("=" * 80)

    # Check current sign distribution BEFORE applying signs
    print(f"\nBEFORE sign application:")
    print(f"  Recurrent: {np.sum(rec_weights > 0)} pos, {np.sum(rec_weights < 0)} neg, {np.sum(rec_weights == 0)} zero")
    print(f"  Input: {np.sum(inp_weights > 0)} pos, {np.sum(inp_weights < 0)} neg, {np.sum(inp_weights == 0)} zero")

    # Apply recurrent weight signs
    if 'recurrent_weights_sign' in model_vars:
        rec_sign = model_vars['recurrent_weights_sign']
        print(f"\nApplying recurrent weight signs...")
        print(f"  Sign mask: {np.sum(rec_sign)} positive, {np.sum(~rec_sign)} negative")

        # Apply: positive sign → keep positive, negative sign → make negative
        rec_weights = np.abs(rec_weights) * np.where(rec_sign, 1.0, -1.0)

        print(f"  AFTER: {np.sum(rec_weights > 0)} pos, {np.sum(rec_weights < 0)} neg")
    else:
        print("\n⚠️  WARNING: No recurrent weight sign mask found!")
        print("  Weights may not follow Dale's law correctly")

    # Apply input weight signs
    if 'input_weights_sign' in model_vars:
        inp_sign = model_vars['input_weights_sign']
        print(f"\nApplying input weight signs...")
        print(f"  Sign mask: {np.sum(inp_sign)} positive, {np.sum(~inp_sign)} negative")

        # Apply: positive sign → keep positive, negative sign → make negative
        inp_weights = np.abs(inp_weights) * np.where(inp_sign, 1.0, -1.0)

        print(f"  AFTER: {np.sum(inp_weights > 0)} pos, {np.sum(inp_weights < 0)} neg")
    else:
    ...
    if model_vars:
        ...
        print(f"  Recurrent: {np.sum(rec_weights > 0)} pos, {np.sum(rec_weights < 0)} neg")
        print(f"  Input: {np.sum(inp_weights > 0)} pos, {np.sum(inp_weights < 0)} neg")
    else:
